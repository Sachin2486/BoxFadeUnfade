...
function App() {
    ...
    const boxes = [0, 1, 2].map((i) => {
      ...
      return [0, 1, 2].map((j) => {
        ...
        if (!(i === 1 && j > 0)) {
          ...
          return <div style={{
            backgroundColor: boxState?.find((item) => item.i === i && item.j === j)?.isClicked && "green",
  ...
  const changeColor = (i, j) => {
    ...
    temp.sort((a, b) => a.order > b.order ? 1 : -1); //to remove the green patch back after the user clicks back
    setBoxState(temp);
